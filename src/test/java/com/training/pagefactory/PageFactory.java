...
public class PageFactory {
	...
	static {
		try {
			...
			getClassInPackage(new com.training.utilities.CommonUtilities().getproperty("page.packages"));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public BasePage initialize(String pageName) {
		...
		try {
			...
			if (packageName != null) {
				...
				System.out.println(
						"Page Class with" + formattedPageName + "doesnt exist .Please provide valid page Name");
			}
	...
	private static void getClassInPackage(String packageName) {
		...
		String classPath = packageName.replaceAll("\\.", Matcher.quoteReplacement(File.separator));
		String srcPath = System.getProperty("user.dir") + "/src/test/java";
		String[] classPathDirs = srcPath.split(System.getProperty("path.separator"));
		String name;
